	...
	class Fields
    {
        ...
        public static function get_pricing_options() {
            ...
            $options = array(
                ...
                'fx'     => array( 'label' => __('Dynamic formula (Pro only)', 'sw-wapf'), 'pro' => true ),
                'percent'   => array( 'label' => __('Percentage based (Pro only)', 'sw-wapf'), 'pro' => true ),
                'nr'   => array( 'label' => __('Amount &times; field value', 'sw-wapf'), 'pro' => true ),
                'char'   => array( 'label' => __('Amount &times; character count', 'sw-wapf'), 'pro' => true ),
            );
	    ...
	    public static function get_raw_field_value_from_request(Field $for_field, $clone_index = 0, $return_null = false) {
		    ...
		    return $_REQUEST['wapf'][$field_name];
	    }
        ...
        public static function value_to_string(Field $field, $raw_value, $include_price_label = true) {
                ...
                if(!empty($field->options['choices'])) {
                    ...
                    foreach ((array) $raw_value as $rv) {
                        ...
                        if($choice['pricing_type'] === 'none')
                            $labels[] = $choice['label'];
                        else $labels[] = sprintf('%s (%s)', esc_html($choice['label']), Helper::format_pricing_hint($choice['pricing_type'],$choice['pricing_amount']));
